...
sub print_body
{
  ...
  my $run_script = "run_redstar.canon.3pt.pl";
...
sub make_script_ib_c9g
{
  ...
  my $fhandle = "gpu.c9g.csh";
  my $id = $self->generate_job_name($outpath) . "_c9g";
  my $dt = $self->generate_dt($outpath); 
...
sub make_script_ib_c9q
{
  &err_func(__func__,__LINE__,"expected 1 arg") unless $#_ == 1; 
  my ( $self, $outpath ) = @_; 

  my $fhandle = "ib.c9q.csh";
  my $id = $self->generate_job_name($outpath) . "_c9q";
  my $dt = $self->generate_dt($outpath); 
...
sub make_script_ib_c10g
{
  ...
  my $fhandle = "gpu.c10g.csh";
  my $id = $self->generate_job_name($outpath) . "_c10g";
  my $dt = $self->generate_dt($outpath); 
...
sub make_script_ib_c9q
{
  ...
  my $fhandle = "ib.c10q.csh";
  my $id = $self->generate_job_name($outpath) . "_c10q";
  my $dt = $self->generate_dt($outpath); 
...
sub make_script
{
  ...
  $hash{"ib_c9q"} = \&QSub::make_script_ib_c9q;
  $hash{"ib_c10q"} = \&QSub::make_script_ib_c10q;
