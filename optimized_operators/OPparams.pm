...
# perl is not strongly typed, abuse this malfeature by returning 
# a filename to a local ensemble when we calculate the weight
sub calculate_post_proj_weight
{
  my ($self,$massf) = @_;  
  my $recon_dir = $self->recon_dir(); 

  my $factor = "proj_factor.jack";

  my $t0 = $self->t0(); 
  my $tz = $self->tz(); 
  my $state = $self->state(); 
  my $projection = "$recon_dir/projections/project_t0${t0}_tZ${tz}/C${state}_${state}.jack";
  if( ! -f $projection ) 
  {
    print "MISSING FILE: $projection\n  -->setting proj weight to 1 \n"; 
    $factor = "1.";
  }
  else
  {
    ...
    symlink $projection , $g; 
    
    my $cmd = "calcbc ' sqrt(  ( exp( $massf * $tz ) * extract( $g , $tz) ) / 2.0 / $massf ) ' | awk '{ print \$2}'";
    # print $cmd . "\n";
    $factor = `$cmd`;
    chomp $factor;
  }
