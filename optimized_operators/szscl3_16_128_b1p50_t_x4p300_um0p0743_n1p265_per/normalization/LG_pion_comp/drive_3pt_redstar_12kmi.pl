...
die "usage: $0 <seqno> <delta_t> <output_path> <npt.list.xml> " unless $#ARGV == 3; 
...
# write the xml we are going to need
my $redxml = $param->write_redstar_xml(); 
my $mesonxml = $param->write_meson_hadron_node_ini_xml();

## need to delete stuff when running local
my $scratch_corr = $param->scratch_seq_callback("output_sdb"); 
unlink $scratch_corr unless ! -f $scratch_corr;

my $smear = $param->scratch_seq_callback("smeared_hadron_node_sdb"); 
unlink $smear unless ! -f $smear; 
...
my $dest_corr = "./D2A2_pion.sdb" . $param->seqno();
unlink $dest_corr unless ! -f $dest_corr;

system ( "cp $scratch_corr $dest_corr") == 0 || die ( $_ ); 
